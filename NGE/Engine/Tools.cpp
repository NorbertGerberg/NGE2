...
eTransformation3D eTools::Decompose(mat4 mat)
{
	...
	vec3 dcEulaRot = eMath::eulerAngles(rotation);
	dcEulaRot = eMath::degrees(dcEulaRot);
...
const vec3 eTools::ColorToVec3(const eColor& color)
{
	return vec3(color.mRed, color.mGreen, color.mBlue);
}

const vec4 eTools::Color4ToVec4(const eColor4& color)
{
	return vec4(color.mRed, color.mGreen, color.mBlue, color.mAlpha);
}

const eColor eTools::Vec3ToColor(const vec3& vl)
{
	eColor rtVl;
	rtVl.mRed	= vl.x;
	rtVl.mGreen = vl.y;
...
const eColor4 eTools::Vec4ToColor4(const vec4& vl)
{
	eColor4 rtVl;
	rtVl.mRed	= vl.x;
	rtVl.mGreen = vl.y;
...
mat4 eTools::GetWorldTransform(eTransformation3D& parent, eTransformation3D& child)
{
	eTransformation3D par = parent;
	par.mRotation = eMath::radians(par.mRotation);

	mat4 rootTransform = eMath::translate(mat4(1.0f), par.mPosition)
		* eMath::mat4_cast(quat(par.mRotation))
		* eMath::scale(mat4(1.0f), par.mScale);

	eTransformation3D childBase = child;
	childBase.mRotation = eMath::radians(childBase.mRotation);

	mat4 childTransform = eMath::translate(mat4(1.0f), childBase.mPosition)
		* eMath::mat4_cast(quat(childBase.mRotation))
		* eMath::scale(mat4(1.0f), childBase.mScale);

	mat4 rtVl = rootTransform * childTransform;
	return rtVl;
}

eTransformation3D eTools::GetWorldTransformAlt(eTransformation3D& parent, eTransformation3D& child)
{
	return Decompose(GetWorldTransform(parent, child));
}