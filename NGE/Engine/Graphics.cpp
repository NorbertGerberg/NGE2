...
eGraphics::eGraphics(HWND window, uint32 width, uint32 height, bool vsync, uint8 msaa, eRenderAPI api)
{
	...
	//mScissorInfo.mPosition = vec2i(0);
	//mScissorInfo.mSize = vec2i(width, height);
	//ApplyScissor();
...
const int eGraphics::DrawInsBeginAtlas(eLayer* layer, std::vector<eAtlasInstance>& instances, bgfx::InstanceDataBuffer& idb)
{
	const int insCnt = instances.size();
	...
	for (uint32 i = 0; i < drawnIns; i++)
	{
		auto& transformation = instances[i].mTransformation;
		real3 posX = (layer->GetProperties().mResolution.x / transformation.mScale.x) / (layer->GetProperties().mResolution.x / transformation.mPosition.x);
		real3 posY = (layer->GetProperties().mResolution.y / transformation.mScale.y) / (layer->GetProperties().mResolution.y / transformation.mPosition.y);
		...
		vec4 subImg = vec4(instances[i].mSubImg, 0.0f, 0.0f);
		memcpy(data, eMath::value_ptr(subImg), sizeof(vec4));
		data += sizeof(vec4);
	}
...
const int eGraphics::Draw2DAtlasInstances(eShader* shader, eLayer* layer, eViewport2D vp, std::vector<eAtlasInstance>& instances, eTextureAtlas& atlas)
{
	if (atlas.mTexture == nullptr || shader == nullptr || layer == nullptr)
		return -1;

	vec4 atinf[2];
	atinf[0] = vec4(0.0f, 0.0f, atlas.mSize);
	atinf[1] = vec4(atlas.mSubSize, 1.0f, 0.0f);
	shader->SetUniform("atlasInfo", atinf, 2);
	...
	bgfx::InstanceDataBuffer idb;
	const int rtVl = DrawInsBeginAtlas(layer, instances, idb);

	SetState(BGFX_STATE_WRITE_RGB | BGFX_STATE_WRITE_A | BGFX_STATE_BLEND_ALPHA);
	ApplyScissor();
	shader->ApplyVertexBuffer(0, mVbh2D);
	bgfx::setInstanceDataBuffer(&idb);
	shader->SetTexture(0, "s_texColor", atlas.mTexture);
	shader->Submit(layer->GetProperties().mViewId, true);

	return rtVl;
}

void eGraphics::Draw2DAtlasInstances(eShader* shader, eLayer* layer, eViewport2D vp, eIDB& idb, eTextureAtlas& atlas)
{
	if (atlas.mTexture == nullptr || shader == nullptr || layer == nullptr) return;
...
void eGraphics::InitIDBAtlas(eLayer* layer, std::vector<eAtlasInstance>& instances, eIDB& idb)
{
	idb.mCouldNotDraw = DrawInsBeginAtlas(layer, instances, idb.mBuffer);
}
...
void eGraphics::DrawString(eShader* shader, eLayer* layer, eViewport2D vp, eTransformation2D transformation,
	eTextAtlas& atlas, eString text, eColor4 color)
{
	eTransformation2D trans = transformation;
	std::vector<eAtlasInstance> instances;
	...
	for (auto& it : stext)
	{
		...
		case 'f': { goNear = true; subImg.x = 5.0f; }break;
		case 'g': { goDown = true; subImg.x = 6.0f; }break;
		case 'h': { subImg.x = 7.0f; }break;
		case 'i': { goNear = true; subImg.x = 8.0f; }break;
		case 'j': { goNear = true; goDown = true; subImg.x = 9.0f; }break;
		case 'k': { subImg.x = 10.0f; }break;
		case 'l': { goNear = true; subImg.x = 11.0f; }break;
		case 'm': { subImg.x = 12.0f; }break;
		case 'n': { subImg.x = 13.0f; }break;
		case 'o': { subImg.x = 14.0f; }break;
		case 'p': { goDown = true; subImg.x = 15.0f; }break;
		case 'q': { goDown = true; subImg.x = 16.0f; }break;
		case 'r': { goNear = true; subImg.x = 17.0f; }break;
		case 's': { subImg.x = 18.0f; }break;
		case 't': { goNear = true; subImg.x = 19.0f; }break;
		...
		case 'I': { goNear = true; subImg = vec2(8.0f, 1.0f); }break;
		...
		case ',': { goNear = true; goDown = true; subImg = vec2(10.0f, 2.0f); }break;
		case ';': { goNear = true; goDown = true; subImg = vec2(11.0f, 2.0f); }break;
		case '.': { goNear = true; subImg = vec2(12.0f, 2.0f); }break;
		case ':': { goNear = true; subImg = vec2(13.0f, 2.0f); }break;
		case '-': { subImg = vec2(14.0f, 2.0f); }break;
		case '_': { subImg = vec2(15.0f, 2.0f); }break;
		case '`': { goNear = true; subImg = vec2(16.0f, 2.0f); }break;
		case '´': { goNear = true; subImg = vec2(17.0f, 2.0f); }break;
		case '<': { subImg = vec2(18.0f, 2.0f); }break;
		case '>': { subImg = vec2(19.0f, 2.0f); }break;
		case '|': { goNear = true; subImg = vec2(20.0f, 2.0f); }break;
		case '!': { goNear = true; subImg = vec2(21.0f, 2.0f); }break;
		...
		case '\'': { goNear = true; subImg = vec2(11.0f, 3.0f); }break;
		case '+': { subImg = vec2(12.0f, 3.0f); }break;
		case '~': { subImg = vec2(13.0f, 3.0f); }break;
		case '*': { goNear = true; subImg = vec2(14.0f, 3.0f); }break;
		case '^': { goNear = true; subImg = vec2(15.0f, 3.0f); }break;
		case '°': { goNear = true; subImg = vec2(16.0f, 3.0f); }break;
		...
		real3 oldY = trans.mPosition.y;
		if (goDown)
			trans.mPosition.y += trans.mScale.y * atlas.mSpace.y * 0.12f;

		eAtlasInstance instance;
		instance.mSubImg = subImg;
		instance.mTransformation = trans;
		instances.push_back(instance);

		if(goNear)
			trans.mPosition.x += trans.mScale.x * 0.5f * atlas.mSpace.x;
		else
			trans.mPosition.x += trans.mScale.x * atlas.mSpace.x;

		trans.mPosition.y = oldY;
	}
	shader->SetUniform("color", eMath::value_ptr(eTools::Color4ToVec4(color)));
	Draw2DAtlasInstances(shader, layer, vp, instances, atlas.mAtlas);
}
...
{
	bool isS = false;
	eShaderMsgBack msg = CompileShaderSingle(name);
...
eShaderMsgBack eGraphics::CompileShaderSingle(eString name, bool vk)
{
...
strg eGraphics::CompileShaderTask(strg arguments)
{
	...
	if (!pipe)
	{
		eConsole::PrintLog("ERROR::eGraphics::CompileShaderTask", "Error creating process!");
		return "ERROR";
	}
	...
	if (_pclose(pipe) == -1)
	{
		eConsole::PrintLog("ERROR::eGraphics::CompileShaderTask", "Error closing process!");
		return "ERROR";
	}
...
void eGraphics::ApplyScissor()
{
	if (mScissorInfo.mSize == vec2i(0))
	{
		bgfx::setScissor();
		return;
	}
	bgfx::setScissor(mScissorInfo.mPosition.x, mScissorInfo.mPosition.y, mScissorInfo.mSize.x, mScissorInfo.mSize.y);
}

void eGraphics::ResetScissor()
{
	bgfx::setScissor();
	mScissorInfo = { vec2i(0), vec2i(0) };
}