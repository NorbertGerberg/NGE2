...
const bool eKeyboard::GetAnyKeyDown()
{
	for (int key = NGE_INPUT_KEY_SPACE; key <= NGE_INPUT_KEY_LAST; key++)
	{
		if (glfwGetKey(GetGLFWwindow(), key) == GLFW_PRESS)
			return true;
	}
...
const bool eKeyboard::GetAnyKeyPressed()
{
	for (int key = NGE_INPUT_KEY_SPACE; key <= NGE_INPUT_KEY_LAST; key++)
	{
		if (glfwGetKey(GetGLFWwindow(), key) == GLFW_PRESS)
...
void eKeyboard::SetKeyCallback(NGE_KEYBOARD_KEYCALLBACK callback)
{
	glfwSetKeyCallback(GetGLFWwindow(), callback);
}
