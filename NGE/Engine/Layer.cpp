...
eLayer::eLayer(bool useFramebuffer)
{
	...
	bUseFramebuffer = useFramebuffer;
}
...
bool eLayer::Initialize()
{
	...
	if (mProperties.bTopMost)
		flags |= BGFX_CLEAR_DEPTH;
	if (!mProperties.bTransparent)
...
{
	if (!bgfx::isValid(mFramebuffer.mFbh) && bUseFramebuffer)
	{
		...
		else if (mProperties.bDepthOnly)
		{
