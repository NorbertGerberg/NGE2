...
namespace ImGui
{
    ...
    IMGUI_API void          Separator(bool vertical = false);                               // separator, generally horizontal. inside a menu bar or in horizontal layout mode, this becomes a vertical separator.
...
{
    template<typename T>
    IMGUI_API bool InputIntExt(const char* label, T* v, ImGuiDataType_ type, int step = 1, int step_fast = 100, ImGuiInputTextFlags flags = 0)
    {
        const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
        return InputScalar(label, type, (void*)v, (void*)(step > 0 ? &step : NULL), (void*)(step_fast > 0 ? &step_fast : NULL), format, flags);
    }

    bool ColorPickerU32(const char* label, ImU32* color, ImGuiColorEditFlags flags = 0);
}
