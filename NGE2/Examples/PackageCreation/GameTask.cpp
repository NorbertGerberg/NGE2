...
bool GameTask::Initialize()
{
	...
	//	Create new package
	mContent->CreatePackage("", "MyPack");
	mContent->LoadPck("", "MyPack");

	//	Insert two textures
	ePackage* pck = mContent->GetPackage("MyPack");

	eFileInfo info = { "test_cursor.png", "test_cursor.png" };
	std::vector<eFileInfo> infos;
	infos.push_back(info);

	pck->InsertAsset("", "MyPack", infos, "test_cursor", "", "texture");

	infos.clear();
	info = { "test_texture.png", "test_texture.png" };
	infos.push_back(info);

	pck->InsertAsset("", "MyPack", infos, "test_texture", "", "texture");
